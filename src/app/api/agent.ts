...
axios.defaults.baseURL =
  process.env.API_URL ?? "https://dev-ic2c.meest.com/api/v1";
...
const Auth = {
  login: (firebase_uid: string, firebase_token: string, device_uuid: string) =>
    requests
  ...
  socialLogin: (
    ...
    requests
      .post<ApiResponse>("/social-login", {
        firebase_uid,
        firebase_token,
        device_uuid,
      } as AuthRequest)
      .then((authResponse) => authResponse.data?.token),
  register: (firebase_uid: string, device_uuid: string) =>
    requests.post<ApiResponse>("/register", {
      firebase_uid,
      device_uuid,
    } as AuthRequest),
...
const Profile = {
  profile: () =>
    requests
      .get<ApiResponse>("/profile")
      .then((authResponse) => authResponse.data?.user),
  updateContactInfo: (request: UpdateContactInfoRequest) =>
    requests
      .post<ApiResponse>("/profile/update-contact-info", request)
      .then((authResponse) => authResponse.data?.user),
};
